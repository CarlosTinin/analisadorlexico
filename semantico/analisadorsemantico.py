...
def analiseSemantica():
	...
	for input_file in lexicalList:
		...
		result[input_file] =proc.semantic_errors_list()
		save_semantic_file(input_file, proc.semantic_errors_list())
		save_syntactic_file(input_file, proc.syntactic_errors_list())
		proc.reset()
		

def save_semantic_file(file_name, errors_list):
    file = open("../files/semantic-outputs/"+file_name, 'w+', encoding = "UTF-8")
    if not errors_list:
...
def save_syntactic_file(file_name, errors_list):
